...
- Replace GNU sed BRE extensions (`\s\+`) with POSIX ERE (`sed -nE`, `[[:space:]]+`) in `git-branch-guard.sh` — fixes hook silently extracting an empty subcommand on macOS BSD sed, which caused every git operation to be blocked during `/claudikins-kernel:execute` (allowlisted commands such as `git add`, `git commit`, `git status` all fell through to the `*)` block and were rejected)
